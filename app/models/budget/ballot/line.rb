class Budget
  class Ballot
    class Line < ActiveRecord::Base
      ...
      validates :ballot_id, :investment_id, :budget_id, presence: true
      ...
      scope :current, ->(investment_id) { where(investment_id: investment_id) }
      scope :counter, ->(investment_id) { where(investment_id: investment_id).count() }

      before_validation :set_denormalized_ids

      def check_sufficient_funds
        errors.add(:money, "insufficient funds") if ballot.amount_available(investment.heading) < investment.price.to_i unless budget.is_new?
      end
