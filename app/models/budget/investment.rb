class Budget
  class Investment < ActiveRecord::Base
    ...
    has_many :likes
    has_many :budget_ballot_lines, foreign_key: 'investment_id'
    ...
    validates :heading_id, presence: true, if: Proc.new { |i| i.budget_id < 10 }
...
def self.get_online_votes

  subquery = %Q{
    SELECT bbl.investment_id, COUNT(*) as total_online_votes
    FROM budget_ballot_lines bbl
    INNER JOIN budget_ballots bb
      ON bb.id = bbl.ballot_id
    INNER JOIN users u
      ON u.id = bb.user_id 
    WHERE u.erased_at IS NULL
    AND bb.budget_id = 11
    GROUP BY bbl.investment_id
    ORDER BY investment_id;
}.squish

  ActiveRecord::Base.connection.execute(subquery)
end
    ...
    scope :by_sector_v2, ->(sector) { joins(:author).where(users: { sector: sector }) }
    scope :search_v2, ->(query) { joins(:author).where("CAST(budget_investments.id AS VARCHAR) ILIKE :query OR title ILIKE :query OR description ILIKE :query OR users.username ILIKE :query OR users.born_names ILIKE :query OR users.paternal_last_name ILIKE :query OR users.maternal_last_name ILIKE :query", query: "%#{query}%") }
    ...
    def total_votes_v2
      Budget::Ballot::Line.counter(id) + self.ballot_offline_count
    end
    ...
    def to_vote? (budget, user)
      budget_id = budget.id && feasible? && user&.sector == self.author.colonium.first.sector && !user&.is_minor? && user.level_three_verified?
    end
    ...
    def new_voted_in?
      ...
      Budget::Ballot::Line.counter(id) > 0
    end

    def new_voted_in_v2?(budget, user)

      # Votos del usuario
      ballots = Budget::Ballot.where(user_id: user&.id, budget_id: budget.id)

      return false if ballots.count == 0

      ballots.each do |ballot|
        voted = Budget::Ballot::Line.find_by(ballot_id: ballot.id, investment_id: self.id)

        return true unless voted.nil?
      end

      #Budget::Ballot::Line.where(budget_id: 11)
      #Budget::Ballot::Line.counter(id) > 0
      #Budget::Ballot.joins(lines: :investment).where(user: user, budget: budget, 'budget_ballot_lines.budget_id' => budget.id).nil?
    ...
    def enough_money?(ballot)
      available_money = ballot.amount_available(heading) unless Budget.find_by(id: budget_id).is_new?
      price.to_i <= available_money unless Budget.find_by(id: budget_id).is_new?
    end
    ...
    def milestone_info
      milestones.order_by_publication_date.last
    end
